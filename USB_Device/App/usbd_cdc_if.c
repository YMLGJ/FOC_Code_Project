...
void cdc_vcp_data_rx(uint8_t *buf, uint32_t Len)
{
    ...
    for (i = 0; i < Len; i++)
    {
        ...
        {
            if (g_usb_usart_rx_sta & 0x4000)        /* 接收到0x0d */
            {
                if (res != 0x0a)
                {
                ...
                {
                    g_usb_usart_rx_sta |= 0x8000;   /* 接收完成 */
                }
            }
            else    /* 还没有收到0x0D */
            {
                ...
                else
                {
                    ...
                    {
                        g_usb_usart_rx_sta = 0;     /* 缓冲区溢出,重新开始 */
                    }
                }
            }
        }
...
/**
 * @brief  通过 USB CDC 发送数据（VOFA 使用此函数）
 */
...
/**
 * @brief  通过 USB CDC 格式化输出（类似 printf）
 */
